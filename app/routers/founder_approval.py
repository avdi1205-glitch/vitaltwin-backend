...
def _apply_entity_side_effect(proposal: dict, new_status: str, admin_email: str) -> None:
    """Only two proposal types represent a real go/no-go business decision
    with an underlying row to flip — everything else is tracking-only."""
...
async def update_approval_status(approval_id: str, data: StatusInput, authorization: str | None = Header(default=None)):
    ...
    _apply_entity_side_effect(proposal, data.status, admin.email)
    record_audit_event(
